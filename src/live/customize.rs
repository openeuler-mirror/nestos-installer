...
pub(super) struct OsFeatures {
    /// Installer reads config files from /etc/nestos/installer.d
    ...
    /// Live initrd reads NM keyfiles from /etc/nestos-firstboot-network
...
pub(super) struct LiveInitrd {
    ...
    /// The nestos-installer config for our own parameters, excluding custom
...
impl LiveInitrd {
    ...
    fn install_hook(
        ...
        live.add_unit(
            ...
            format!(
                "# Generated by nestos-installer {{iso|pxe}} customize
...
RequiredBy={install_target}",
    ...
    pub fn into_initrd(mut self) -> Result<Initrd> {
        if self.dest.is_some() || !self.user_dest.is_empty() {
            ...
            let dest_path = "/etc/nestos/dest.ign";
            self.live.get_or_insert_with(Default::default).add_file(
                dest_path.into(),
                &data,
                0o600,
